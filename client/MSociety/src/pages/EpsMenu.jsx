...
const EpsMenu = () => {
    ...
    const [activeSeason, setActiveSeason] = useState(1); // Retrieve from storage or default to 1
    ...
    const closeBtnNavigate = () => {
        ...
        const basePath = currentPath.substring(0, currentPath.lastIndexOf(':seasonNumber/episodes'));
    ...
    return (
            <StyledContainer>
                <IoMdCloseCircle className={' absolute left-4 top-4 lg:mt-[100px] lg:ml-[50px] lg:w-[3.5em] lg:h-[3.5em] w-[3em] h-[3em] text-white cursor-pointer'} onClick={closeBtnNavigate}/>
                                ...
                                for (let i = 1; i <= seasonData.length; i++) {
                                    seasons.push(
                                        <a
                                            key={i}
                                            id={i}
                                            className={` font-medium cursor-pointer lg:pt-[20px] pt-3 lg:text-3xl text-2xl ${activeSeason === i ? ' text-white' : ' text-gray-600'}`}
                                            onClick={() => {
                                                handleSeasonClick(i);
                                            }}
                                        >
                ...
                <h3 className={' text-white lg:text-3xl text-2xl lg:mb-[40px] mb-5'}>الحلقات</h3>
                    {episodesData.map(episode => {
                        return(
                            <Episode key={episode.episodeNumber} episodeNumber={episode.episodeNumber}
                            title={episode.title} description={seriesData.description} />
                        )
                    })} 
                </div>
            </StyledContainer>
