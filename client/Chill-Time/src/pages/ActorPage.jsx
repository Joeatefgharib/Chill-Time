...
const ActorPage = () => {
    ...
    useEffect(() => {
        axios.get(`http://89.116.110.212:5000/api/actors/${id}`)
        .then(response => {
            setActorData(response.data);

            axios.get(`http://89.116.110.212:5000/api/actors/${id}/relatedMovies`)
            .then(async response => {
                const moviesData = await Promise.all(response.data.map(async movieId => {
                    const movieResponse = await axios.get(`http://89.116.110.212:5000/api/movies/${movieId}`);
                    console.log(movieResponse.data)
                    return movieResponse.data;
            }));
                setMovieData(moviesData); // Update movieData state with movie details
            })
            axios.get(`http://89.116.110.212:5000/api/actors/${id}/relatedSeries`)
            .then(async response => {
                const seriesData = await Promise.all(response.data.map(async SeriesId => {
                    const SeriesResponse = await axios.get(`http://89.116.110.212:5000/api/series/${SeriesId}`);
