...
const SeriesPage = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://89.116.110.212:5000/api/series/${id}/actors`)
      .then((response) => {
        console.log(response.data);
        ...
        const actorDetailsPromises = response.data.map((actorId) =>
          axios
            .get(`http://89.116.110.212:5000/api/actors/${actorId}`)
  ...
  return (
    <>
      <Helmet>
        <title>
          مشاهدة مسلسل {`${seriesData.title}`} بجودة عالية على Chill Time
        </title>
        <meta
          name="keywords"
          content={`تحميل ومشاهدة مسلسل ${seriesData.title} مترجم`}
        />
      ...
      <StyledContainer>
        ...
        <div
          id="seriesContent"
          className="lg:flex lg:items-center lg:justify-between grid gap-10 "
        >
          <img
            id="img"
            className="w-[240px] h-[355px] rounded-2xl lg:mr-[100px] lg:mt-0 mr-[75px] mt-[100px]"
            alt={`${seriesData._id}`}
          ...
          <div id="seriesInfoContent" className=" h-[360px] mr-[50px] pt-8">
            ...
            <p className="text-gray-500 mt-[10px] w-4/6">
